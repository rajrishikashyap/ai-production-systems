...
|---|---|---|
| **MyClaw** | Main AI agent that runs the pipeline 24/7 | Persistent agent that orchestrates the full workflow autonomously |
...
**Step 5: Log + Track**
All outcomes - sent, opened, replied, bounced and sync back to Airtable. The team has a live view of pipeline health without touching any of the underlying tools.
...
**Personalisation at scale requires constraints.** Claude generating completely freeform personalisation produced inconsistent quality. I had to design a structured prompt with explicit boundaries, what to reference, what tone to use and maximum length before the output was reliable enough for production.

**Airtable as the source of truth.** Every tool in the stack has its own dashboard. Having Airtable as the single interface the team actually uses, with automated views, filtered pipelines, and status tracking that was what made the system usable by non-technical people.
